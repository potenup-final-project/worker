...
class MessageLogExecutor(
    ...
    private fun logStart(context: WorkerMessageContext) {
        log.info(
            serialize(
                linkedMapOf(
                    "logType" to "MESSAGE",
                    WorkerLogPayloadKeys.PHASE to "START",
                    WorkerLogPayloadKeys.TRACE_ID to context.traceId,
                    "orderFlowId" to context.orderFlowId,
                    WorkerLogPayloadKeys.EVENT_TYPE to context.eventType,
                    "consumer" to context.consumer,
                    WorkerLogPayloadKeys.MESSAGE_ID to context.messageId,
                    "queue" to context.queue,
                    "topic" to context.topic,
                    WorkerLogPayloadKeys.RETRY_COUNT to context.retryCount,
    ...
    private fun logEnd(
        ...
        val payload = linkedMapOf(
            "logType" to "MESSAGE",
            WorkerLogPayloadKeys.PHASE to "END",
